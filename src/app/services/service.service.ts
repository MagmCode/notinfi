...
//#region Utilitarios / tareas

consultaProcesos(data: { fechaDesde: string; fechaHasta: string; enEjecucion: number; userId: string }): Observable<any> {
  return this.http.post(`${this.apiUrl}api/procesos/consultar`, data).pipe(
    map((response: any) => {
      return response;
    })
  );
