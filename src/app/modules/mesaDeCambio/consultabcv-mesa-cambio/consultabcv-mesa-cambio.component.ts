...
import { ConsultaBcv } from 'app/models/mesa-cambio';
...
export class ConsultabcvMesaCambioComponent implements OnInit,  AfterViewInit, OnDestroy {
  selectedRow: any;
  /** Diccionario para mostrar nombres personalizados en los encabezados de la tabla */
  columnNames: { [key: string]: string } = {
    'ID': 'ID Operación',
    'TIPO_OPER': 'Tipo ConsultaBcv',
...
//#region Variables
  ...
  operaConsultaBcvForm: FormGroup;
  ...
  selection = new SelectionModel<ConsultaBcv>(true, []);
  ...
  {
    this.dataSourceH = new MatTableDataSource(this.ConsultaBcv); 
  }
  ...
  onTabChange(event: any): void {
    if (event.index === 0) {
      localStorage.removeItem('operacionesConsultaBcvBusqueda');
      localStorage.removeItem('operacionesConsultaBcvResultados');
      localStorage.removeItem('operacionesConsultaBcvCantidad');
      localStorage.removeItem('operacionesConsultaBcvTotal');
      localStorage.removeItem('operacionesConsultaBcvJornada');
   ...
   */
  ngOnInit(): void {
    const savedBusqueda = localStorage.getItem('operacionesConsultaBcvBusqueda');
    if (savedBusqueda) {
      const busqueda = JSON.parse(savedBusqueda);
      this.operaConsultaBcvForm = this._formBuilder.group({
        jornada: [busqueda.jornada, [Validators.required]],
        movimiento: [busqueda.movimiento],
      });

      // Recupera los resultados guardados
      const savedResultados = localStorage.getItem('operacionesConsultaBcvResultados');
      if (savedResultados) {
  ...
  this.ConsultaBcv = safeResultados;
      ...
      const savedCantidad = localStorage.getItem('operacionesConsultaBcvCantidad');
      if (savedCantidad) {
        this.cantidad = JSON.parse(savedCantidad);
      }
      const savedTotal = localStorage.getItem('operacionesConsultaBcvTotal');
      if (savedTotal) {
        this.total = JSON.parse(savedTotal);
      }
      const savedJornada = localStorage.getItem('operacionesConsultaBcvJornada');
      if (savedJornada) {
   ...
   */
  consultarConsultaBcv() {
    this.isLoading = true;
    if (this.operaConsultaBcvForm.controls['fechOper'].invalid) {
    ...
    this.selectedTabIndex = 1;
    this.canViewTab = true;
  this.cantidad = this.ConsultaBcv.length.toString();
  this.total = this.ConsultaBcv.reduce((acc, curr) => acc + (curr.MONTO_ENT || 0), 0).toString();
   ...
   */
  onCheckboxChange(event: MatCheckboxChange, row: ConsultaBcv) {
    if (event.checked) {
...
cerrarLote() {
  ...
  const formFecha = this.operaConsultaBcvForm.get('fechOper')?.value;
...
exportarExcel(): void {
    ...
    this.exportProgressService.iniciarProgreso(
      ...
      () =>  this._service.exportarIntervencion('ConsultaBcvFiltroExportar', this.operaConsultaBcvForm.value)
    );
}
...
applyFilterH(event: Event) {
  ...
  regresar(): void {
    ...
    this.canViewTab = false;
    this.operaConsultaBcvForm.reset();
    // Asigna la fecha del día nuevamente
    const today = new Date();
    this.operaConsultaBcvForm.get('fechOper')?.setValue(today);
