...
import { Canje } from 'app/models/mesa-cambio';
...
@Component({
  selector: 'app-consulta-canje-mesa-cambio',
  templateUrl: './consulta-canje-mesa-cambio.component.html',
  styleUrls: ['./consulta-canje-mesa-cambio.component.scss']
})
export class ConsultaCanjeMesaCambioComponent implements OnInit,  AfterViewInit, OnDestroy {
  selectedRow: any;
  /** Diccionario para mostrar nombres personalizados en los encabezados de la tabla */
  columnNames: { [key: string]: string } = {
    'ID': 'ID Operación',
    'TIPO_OPER': 'Tipo Canje',
...
//#region Variables
  ...
  selection = new SelectionModel<Canje>(true, []);
  ...
  dataSourceH: MatTableDataSource<Canje>;
  Canje: Canje[] = [
    {
      ID: 1,
      TIPO_OPER: 'C',
  ...
  {
    this.dataSourceH = new MatTableDataSource(this.Canje); 
  }
  ...
  onTabChange(event: any): void {
    if (event.index === 0) {
      localStorage.removeItem('operacionesCanjeBusqueda');
      localStorage.removeItem('operacionesCanjeResultados');
      localStorage.removeItem('operacionesCanjeCantidad');
      localStorage.removeItem('operacionesCanjeTotal');
      localStorage.removeItem('operacionesCanjeJornada');
   ...
   */
  ngOnInit(): void {
    const savedBusqueda = localStorage.getItem('operacionesCanjeBusqueda');
    if (savedBusqueda) {
      ...
      const savedResultados = localStorage.getItem('operacionesCanjeResultados');
      if (savedResultados) {
  ...
  this.Canje = safeResultados;
      ...
      const savedCantidad = localStorage.getItem('operacionesCanjeCantidad');
      if (savedCantidad) {
        this.cantidad = JSON.parse(savedCantidad);
      }
      const savedTotal = localStorage.getItem('operacionesCanjeTotal');
      if (savedTotal) {
        this.total = JSON.parse(savedTotal);
      }
      const savedJornada = localStorage.getItem('operacionesCanjeJornada');
      if (savedJornada) {
  ...
  consultarCanje() {
    this.isLoading = true;
    if (this.operaCanjeForm.controls['fechOper'].invalid) {
    ...
    this.selectedTabIndex = 1;
    this.canViewTab = true;
  this.cantidad = this.Canje.length.toString();
  this.total = this.Canje.reduce((acc, curr) => acc + (curr.MONTO_ENT || 0), 0).toString();
   ...
   */
  onCheckboxChange(event: MatCheckboxChange, row: Canje) {
    if (event.checked) {
...
cerrarLote() {
  ...
  const formFecha = this.operaCanjeForm.get('fechOper')?.value;
...
exportarExcel(): void {
    ...
    this.exportProgressService.iniciarProgreso(
      ...
      () =>  this._service.exportarIntervencion('CanjeFiltroExportar', this.operaCanjeForm.value)
    );
}
...
applyFilterH(event: Event) {
  ...
  regresar(): void {
    ...
    this.canViewTab = false;
    this.operaCanjeForm.reset();
    // Asigna la fecha del día nuevamente
    const today = new Date();
    this.operaCanjeForm.get('fechOper')?.setValue(today);
